...
- Issue #737: Home, Log, and Weight now show a loading placeholder instead of a blank screen while data is still being read, and Home, Log, Weight, and Analytics show a retry banner when a read fails instead of presenting an empty or zeroed screen as if it were real. A weight goal that fails to load is now reported as a failure you can retry rather than shown as "no goal set". Home also shows when changes are saved on this device and waiting for Cloud Sync, with a link to Cloud Sync and a retry after a failed sync.
- Issue #738: Closed accessibility gaps across the daily surfaces: collapse targets on the current-routine card and routine list now announce their role, label, and expanded state; Deload's confirmation controls and per-record delete actions are labeled and reachable at a 44dp touch target; Recovery inclusion switches now speak which block they belong to; Home's Pace stat and decorative trend chevrons no longer rely on color alone; and hidden (inactive) tabs are excluded from the screen-reader navigation order.
- Issue #748: Saving a routine no longer leaves you stranded: a prompt offers to make it your current routine, the Log tab now shows a clear next step whenever no routine is current or none has been logged yet, guided routine creation and Copy last session cut retyping, and the empty-state and Home copy now teach the format truthfully.
- Issue #751: The fatigue check-in now only asks about sessions there is real evidence for, and only when you finish one. Pressing Skip week, or logging a whole day as skipped, no longer produces a "you okay?" prompt — a skip is taken as your statement that nothing happened. Leaving the Log tab mid-edit no longer prompts either, so the check-in can never ask about work that was not saved. Straight sets that lose a rep or two near failure, back-off sets below the day's top weight, and a second-ever session at a new weight are all treated as ordinary training instead of a decline, and "more skipped than usual" is now measured against your own usual rate rather than your best-ever session. After a check-in, the next two sessions are left alone. The prompt no longer appears over a failed or still-loading read, while another dialog is open, or when fatigue tracking is off, and it closes without saving anything if any of those change while it is open. Its wording now describes the session — "Lighter than usual", "Normal for me", "It was a rough one" — instead of asking how you are.
...
- Issue #695: Log now supports starting a recovery block: freeze a baseline routine, then create or attach one note as Recovery Week 1.
- Issue #696: Log now supports advancing and manually completing a recovery block: complete a week before adding the next one, attach a new or existing note as each sequential week, unlink the latest week, complete the whole block, and browse completed blocks in a collapsible history. Completing a block with its open current week, and deleting a linked recovery-week note, are now crash-safe: each records its intent before writing, verifies the result against stored data, and resumes automatically after an interrupted attempt, an app restart, or a sync. Adding a week with a brand-new note is covered the same way, so a double tap or an interrupted attempt can never create a duplicate note or skip a week number; if another device took that week number first the week is renumbered and finishes on retry, and if the change has become impossible Log explains it once instead of leaving recovery stuck. A cloud sync or a first cloud upload can no longer run at the same time as one of these changes, so neither can overwrite the other and signing in never uploads a half-finished change. If a change cannot be finished yet it is shown as pending with a Retry recovery action instead of being reported as done, and a pending change never leaves a recovery week pointing at a deleted note or a deletion that never reaches the cloud after signing in.
- Issue #698: Analytics now has a Recovery section showing the return-to-baseline comparison for your active recovery block, or any completed block selected from a collapsible history: a factual count of baseline exercises met, a week selector, and per-exercise Load/Volume or Reps/Time evidence against your frozen baseline, including exercises added during recovery and values above 100%. This is evidence only — nothing here changes calculations, completes a block, or claims medical recovery.
- Issue #699: Added a per-block "Include recovery notes in normal analytics" switch to the active and completed recovery blocks in the Log tab. It is off by default, so a block's linked recovery weeks stay out of exercise classifications, progressive-overload signals, Kilo Max, non-weighted tracked metrics, 1K history, and Home summaries while remaining fully visible, editable, and present in Recovery Analytics. Turning it on feeds those notes back through the ordinary aggregations, and each block honors its own setting independently. Deload exclusions are unchanged. Restoring a backup now refreshes recovery data on the open screens, and if the recovery records cannot be read, Home and the affected Analytics sections keep loading instead of showing totals that include work you chose to exclude.
