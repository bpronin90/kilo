...
role and its single grant (`execute` on the accessor, and nothing else). One
action remains for an authorized operator and is never committed:
...
repository secret. Until it is set, the scheduled monitor reports red.
