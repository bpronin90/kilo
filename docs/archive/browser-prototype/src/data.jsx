...
(function computeTotal() { const ids = ['squat', 'db_bench', 'deadlift']; let total = 0; for (const id of ids) { const sess = window.KILO_SESSIONS.find(s => s.exercises.some(x => x.exerciseId === id)); const e = sess && sess.exercises.find(x => x.exerciseId === id); const row = e && window.parseWorkoutRow(e.raw); if (row && row.ok) for (const s of row.sets) total = Math.max(total, window.epleyPR(s.weight_value, s.rep_count) || 0); } KILO_GOALS[0].current = Math.round(total); })();
...
window.updateWeightEntry = function(id, weightValue) { const e = window.KILO_WEIGHTS.find(x => x.id === id); if (!e) return false; e.weight = e.weight_value = weightValue; return true; };
window.deleteWorkoutSession = function(id) { window.KILO_SESSIONS = window.KILO_SESSIONS.filter(e => e.id !== id); return true; };
