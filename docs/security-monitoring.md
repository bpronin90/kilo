...
operator, and it is never committed:

```sql
alter role kilo_security_monitor with password '<generated by the operator>';
```

Set that role's session-pooler URL as the `SUPABASE_SECURITY_MONITOR_URL`
repository secret. Until that is done the scheduled monitor exits 2 and reports
red — deliberately. A monitor that cannot see the database must never report a
clean security window.
