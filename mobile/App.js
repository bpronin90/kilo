...
import { ThemeProvider, KuaStyleGate, useTheme } from './theme/ThemeContext';
...
function ShellView({ onDeviceDataWiped }) {
  ...
  // Built from the resolved court palette (kua) and mode rather than
  // useThemedStyles: ShellView owns the outermost canvas/safe-area chrome and
  // the modal scrim, and the scrim is the KUA-spec neutral backdrop keyed on
  // mode, which the two-argument style hook does not carry (#1139).
