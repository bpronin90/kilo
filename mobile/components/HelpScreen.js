...
const TOPICS = [
  ...
  ['logging', 'Log workouts', 'Routine notes, syntax, tracking, and deloads. Track / Tracked: logging it alone never does. Every explicit Track opens a fresh progression span; First session appears while the new span builds, while Est. Max, Kilo Max, and best set keep showing your full history. Exercises may already be tracked from a catalog default or an earlier version of the app.'],
  ['weight', 'Weight', 'Daily weigh-ins, goals, trends, and pace flags.'],
  ['analytics', 'Analytics', 'Weight trend charts and a combined Big 3 progress view for mapped squat, bench, and deadlift when enough complete logged cycles exist. Progressive Overload shows Est. Max, Kilo Max, best set, and progress trend; when fatigue tracking is enabled, check-in data adds context. Recovery compares planned training with baseline.'],
...
export function HelpScreen({ onBack }) {
  ...
  return <ScreenShell title="App Guide" subtitle="A quick reference for using Kilo." onBack={onBack}>
    {!topic ? <>
      ...
      <View style={styles.compatibilityContent} accessibilityElementsHidden importantForAccessibility="no-hide-descendants"><WorkoutSyntaxReference /></View>
    </> : <>
