// The per-table sync loop.
//
// Split out of storage/syncQueue.js (issue #1061). This is the orchestration
// layer that ties every other sync module together. The roadmap loop shape is:
// pull changed rows since cursor -> merge into local cache -> push dirty local
// records -> advance the per-table cursor only after a successful push. The
// cloud transport is injected, so this engine is fully testable without a
// network and without coupling to bootstrap (#319).
//
// It owns no persistent module state of its own: the client id, the monotonic
// clock, the dirty listeners and the snapshot at-rest cache all live in the
// modules that export them, so a spy or reset applied through any single owner
// is seen everywhere.

import { getClientId, mergeRecords, SINGLETON_SYNC_ID, SYNC_TABLES } from './records';
import {
  getCursor,
  normalizePullResult,
  advanceCursorFromServerEvidence,
} from './cursors';
import { enqueueDirtyMany, getDirtyRecords, clearDirty } from './dirtyQueue';
import { getSyncSnapshot, setSyncSnapshot, diffAgainstBaseline } from './snapshots';
import { reconcileAgainstRemote, SyncReconciliationConflictError } from './reconciliation';
