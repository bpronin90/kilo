-- Use the consent-grant rate-limit window in the global pruner (issue #609).
--
...
--   consent_grant:<uuid>     — 1-minute window
...
as $$
  ...
  where occurred_at < case
    ...
    when bucket like 'consent_grant:%' then now() - interval '1 minute'
