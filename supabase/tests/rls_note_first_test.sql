...
select lives_ok(
  $$insert into public.user_profile (user_id) values ('11111111-1111-1111-1111-111111111111')$$,
  'user A can insert own user_profile');
select lives_ok(
  $$insert into public.feature_toggles (user_id) values ('11111111-1111-1111-1111-111111111111')$$,
  'user A can insert own feature_toggles');
select lives_ok(
  $$insert into public.weight_entries (user_id, id, weight_value) values ('11111111-1111-1111-1111-111111111111', 'wa', 80)$$,
  'user A can insert own weight_entries');
select lives_ok(
  $$insert into public.weight_goal (user_id) values ('11111111-1111-1111-1111-111111111111')$$,
  'user A can insert own weight_goal');
select lives_ok(
  $$insert into public.workout_notes (user_id, id) values ('11111111-1111-1111-1111-111111111111', 'na')$$,
  'user A can insert own workout_notes');
select lives_ok(
  $$insert into public.deload_history (user_id, id) values ('11111111-1111-1111-1111-111111111111', 'da')$$,
  'user A can insert own deload_history');
select lives_ok(
  $$insert into public.fatigue_checkins (user_id, id) values ('11111111-1111-1111-1111-111111111111', 'fa')$$,
  'user A can insert own fatigue_checkins');

select pg_temp.login_as(:'user_b'::uuid);

select lives_ok(
  $$insert into public.user_profile (user_id) values ('22222222-2222-2222-2222-222222222222')$$,
  'user B can insert own user_profile');
select lives_ok(
  $$insert into public.feature_toggles (user_id) values ('22222222-2222-2222-2222-222222222222')$$,
  'user B can insert own feature_toggles');
select lives_ok(
  $$insert into public.weight_entries (user_id, id, weight_value) values ('22222222-2222-2222-2222-222222222222', 'wb', 90)$$,
  'user B can insert own weight_entries');
select lives_ok(
  $$insert into public.weight_goal (user_id) values ('22222222-2222-2222-2222-222222222222')$$,
  'user B can insert own weight_goal');
select lives_ok(
  $$insert into public.workout_notes (user_id, id) values ('22222222-2222-2222-2222-222222222222', 'nb')$$,
  'user B can insert own workout_notes');
select lives_ok(
  $$insert into public.deload_history (user_id, id) values ('22222222-2222-2222-2222-222222222222', 'db')$$,
  'user B can insert own deload_history');
select lives_ok(
  $$insert into public.fatigue_checkins (user_id, id) values ('22222222-2222-2222-2222-222222222222', 'fb')$$,
...
select is(
  (select count(*)::int from public.feature_toggles),
...
select is(
  (select count(*)::int from public.weight_goal),
  1, 'user A sees only own weight_goal row');
select is(
  (select count(*)::int from public.workout_notes),
  1, 'user A sees only own workout_notes row');
select is(
  (select count(*)::int from public.deload_history),
  1, 'user A sees only own deload_history row');
select is(
  (select count(*)::int from public.fatigue_checkins),
  1, 'user A sees only own fatigue_checkins row');

-- Direct attempts to read user B's specific rows return nothing.
select is(
  (select count(*)::int from public.weight_entries where id = 'wb'),
  0, 'user A cannot read user B weight_entries row');
select is(
  (select count(*)::int from public.workout_notes where id = 'nb'),
  0, 'user A cannot read user B workout_notes row');
select is(
  (select count(*)::int from public.user_profile where user_id = '22222222-2222-2222-2222-222222222222'),
...
-- ---------------------------------------------------------------------------
select is(
  (with u as (update public.weight_entries set note = 'hacked' where id = 'wb' returning 1) select count(*)::int from u),
  0, 'user A update of user B weight_entries affects no rows');
select is(
  (with u as (update public.workout_notes set raw_text = 'hacked' where id = 'nb' returning 1) select count(*)::int from u),
  0, 'user A update of user B workout_notes affects no rows');
select is(
  (with u as (update public.user_profile set display_name = 'hacked' where user_id = '22222222-2222-2222-2222-222222222222' returning 1) select count(*)::int from u),
  0, 'user A update of user B user_profile affects no rows');
select is(
  (with u as (update public.feature_toggles set deload_mode_enabled = false where user_id = '22222222-2222-2222-2222-222222222222' returning 1) select count(*)::int from u),
  0, 'user A update of user B feature_toggles affects no rows');
select is(
  (with u as (update public.weight_goal set target_weight = 1 where user_id = '22222222-2222-2222-2222-222222222222' returning 1) select count(*)::int from u),
  0, 'user A update of user B weight_goal affects no rows');
select is(
  (with u as (update public.deload_history set raw_text = 'hacked' where id = 'db' returning 1) select count(*)::int from u),
  0, 'user A update of user B deload_history affects no rows');
select is(
  (with u as (update public.fatigue_checkins set status = 'hacked' where id = 'fb' returning 1) select count(*)::int from u),
...
-- ---------------------------------------------------------------------------
select is(
  (with d as (delete from public.weight_entries where id = 'wb' returning 1) select count(*)::int from d),
  0, 'user A delete of user B weight_entries affects no rows');
select is(
  (with d as (delete from public.workout_notes where id = 'nb' returning 1) select count(*)::int from d),
  0, 'user A delete of user B workout_notes affects no rows');
select is(
  (with d as (delete from public.user_profile where user_id = '22222222-2222-2222-2222-222222222222' returning 1) select count(*)::int from d),
  0, 'user A delete of user B user_profile affects no rows');
select is(
  (with d as (delete from public.feature_toggles where user_id = '22222222-2222-2222-2222-222222222222' returning 1) select count(*)::int from d),
  0, 'user A delete of user B feature_toggles affects no rows');
select is(
  (with d as (delete from public.weight_goal where user_id = '22222222-2222-2222-2222-222222222222' returning 1) select count(*)::int from d),
  0, 'user A delete of user B weight_goal affects no rows');
select is(
  (with d as (delete from public.deload_history where id = 'db' returning 1) select count(*)::int from d),
  0, 'user A delete of user B deload_history affects no rows');
select is(
  (with d as (delete from public.fatigue_checkins where id = 'fb' returning 1) select count(*)::int from d),
...
-- ---------------------------------------------------------------------------
select throws_ok(
  $$insert into public.weight_entries (user_id, id, weight_value) values ('22222222-2222-2222-2222-222222222222', 'spoof', 1)$$,
  '42501',
  null,
  'user A cannot insert a row owned by user B');
select throws_ok(
  $$insert into public.workout_notes (user_id, id) values ('22222222-2222-2222-2222-222222222222', 'spoof')$$,
  '42501',
  null,
...
select is(
  (select note from public.weight_entries where id = 'wb'),
